...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** The result is an overview of the file, the dimension, bounding box, projected CRS and a count of rows and columns. The columns include an object ID, zip code, zip code name, another zip code column, "existing", shape_length, shape_area and shape_geometry
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** It looks a lot like the data sets we've seen before with the additional geometry column. It has the zip code, the name describing the zip code and a few notes that will likely go into shaping the data when displayed.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It gives us the state of Maryland. It looks like it was broken up by zip code. It also has latitude and longitude lines.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It installed it, but also made me enter this code "options(tigris_use_cache = TRUE)"
...
**Task** Run the following code. Describe the output in the space below. **Answer** It retrieved data from 2021 and now shows us the columns of the database.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code is entered in the console in blue.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It shows a map of Maryland by county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We get a mix of both the counties and zip codes over the state of Maryland. It's a lot to vizualize though.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We get a chart with two columns: zip codes and foreclosures in March of 2023.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Got an error in the join_by statement so I changed the = to ==. After, it ran smoothly and was able to join the two.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We get a blue map of Maryland -- the lighter the blue, the more foreclosures in the zip code.
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** The libraries are loaded -- since I have already added the census api key in an earlier lab, it should be set.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Two different data sets -- one for the notices that we've read into the code and the second on popultaion by county
...
**Task** Run the following code. Describe the output in the space below. **Answer** The data set is "getting the data from the 2017-2021 5-year ACS using the FIPS code for Maryland. We get a mini table of the data.
...
**Task** Run the following code. Describe the output in the space below. **Answer**It shows the notices data set.
...
**Task** Run the following code. Describe the output in the space below. **Answer** All mentions of "Maryland" have been removed from the County column, the name column has been renamed county, spaces have been taken out of the county column, the estimate column was renamed population, and the data set was narrowed down to just the county, population and geometry.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The two data sets have been joined by the county name but look more like they've been stacked on top of each other.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A new column has been added that shows the notices per 1000 population and is sorted from highest to lowest. There's also still the description clip at the top.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We get a map of Maryland by county with labels that highlights the rate of foreclosures. It's made clear that Charles County has the highest rate.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Different colors better contrast the difference between rates. The labels are gone though.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** There's a higher rate of foreclosure notices in the middle of the state, especially in counties with large cities.
