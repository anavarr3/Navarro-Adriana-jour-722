...
```{r echo=FALSE}
census_api_key("f91af41aeabe12d5ca3ef5bad52c044e9b9a2cbd", install=TRUE)
```
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** Estimated Maryland household income by county, 2017-2021
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer** Average household income for high schools that have at least 60% of graduates enrolling in college is $111,448.30. Meanwhile, the average household income for schools with less than 40% enrollment is $69,252.24 -- a little more than half of the former.
...
**Task** What is another question you could ask of this data? **Answer** I want to know why the 20612 zip code data is unavailable. Also, of the graduates, which high school has the highest percentage of college enrollments? Are the top 5 all in the same county? Bottom 5 all in the same county? This might not be in the data, but are the schools in the top 5 made up of a majority white student body?
