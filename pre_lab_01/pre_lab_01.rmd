...
The added line of code arrange(desc(count_classes)) arranged the count of classes in descending order, or greatest to least. Without the added desc(), the code would have arranged the column in ascending order, or least to greatest
...
**Answer**
The code is now also adding the department in as a column along with the term. For the arrangement, the code prioritizes the term in ascending order and then the department in alphabetical order (still ascending)
...
We are now counting the total seats offered in each term and assigning that total to the variable total_seats; and then we are taking that data and arranging it in descending order by the total seats.
...
We are now finding the mean and median of the number of seats and then arranging the sum of the seats in descending order. The top terms have a staggering number of classes compared to the terms on the majority of pages 2 and 3.
...
I added two columns to show the maximum number of seats and the minimum number of seats across the classes in each department. The data is still sorted by highest number of seats to lowest.
