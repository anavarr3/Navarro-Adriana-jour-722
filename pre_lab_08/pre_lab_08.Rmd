...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer** 3822 rows, 4 columns
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer** Take the original baltco_911_calls data and make a new dataset called "baltco_911_calls_by_month AND THEN add another column (mutate) called "month" and display the month of the date column and display it as a label (Jan., Feb., etc.) AND THEN group the results by the month AND THEN count the total number of calls for each of those months AND THEN arrange in descending order (highest to lowest).
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** A gray box, as mentioned above
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** Take the dataset baltco_911_calls_by_month AND THEN invoke ggplot AND THEN use bars where the make the x axis "months" and the y access the count of total calls. I now see a bar chart where the count is the y access and the month is the x axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Instead of the x-axis being in order by month, it has now been arranged by the count. I see the reorder function, but I'm not sure how it knows to reorder it by count. Unless the part before the comma (month) is telling it "this is the variable to use for x and the second place after the comma (total_calls) is then telling it how to order.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The same bar chart, but now it flips the x and y axis and the orientation of the bars.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** The background changed! The layout mostly stayed the same, but the colors/style/theme changed.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The x and y labels changed to "month" and "total calls." In the code, + labs() was added after the themes function. Within labs, we assigned the title as "More 911 Overdoes Calls in Warmer Months," x = months and y = total calls. We also added a caption as "source: Baltimore County."
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** a date column and a column showing the total calls on that day.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** A line chart with total_calls as the y-axis and dates as the x-axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** Line graph again, but now the dates are all crowded at the bottom.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** They're tilted at an angle now but still pretty crowded
