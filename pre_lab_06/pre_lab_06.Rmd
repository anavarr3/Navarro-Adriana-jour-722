...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** Yes, I believe they do
...
spec(county_voters_2016)
spec(county_voters_2018)
spec(county_voters_2020)
spec(county_voters_2022)
```
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** Democrats see the most growth, though Republican numbers also grow significantly, Libertarians rise, dip and then rise again, Green mostly dips -- overall, voters rise. The last total column and the 2022 entry under the "TOTAL" column are "NA"
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** 24 rows -- it prioritized the maryland_population one, which has 24 rows. It looks weird though since the pop2020 is being applied to 2022 since it's cutting out the remaining years. It should have 96 rows like the other dataframe
...
view(maryland_population_with_voters)
```

### Task 9: Add the Demographic Data

**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** It shows the geocode, the county, a breakdown of the population by race and to a small extent age
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's County -- 60.9% 18+ Black population and 77.1% Democrats in the county
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Prince George's County is a bit lower down now, though of the 4.6% people there of that demographic, over 77% of them are Democrats. However, in Howard County, which has the highest Asian population as percentage of all population at 19.4%, only a little over half of them are Democrats.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer** dbl
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** chr
