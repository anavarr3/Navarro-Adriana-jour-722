...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are some variations in the names, like "Way Station Inc." and "Way Station, Inc."
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** Some grantees that were originally lower case were capitalized. There were also instances where "Inc." had been dropped from a company name. It's possible that these were just merged. 
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** "Washington Adventist University" was made lowercase in the clean column, "II" was taken out of "WALDORF PROSPERITY, LLC." I can't find another difference in the first 30 rows, but on page 5, "Washington County Community Action Council" was made lowercase and no longer had "INC" in the clean column.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** I can't find a difference between the two in the first 30 rows.
