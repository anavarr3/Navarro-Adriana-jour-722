...
**Answer** To store the result and call upon later if we wanted to sort through these courses.
...
**Answer** 41011 -- a much larger number of rows than AND gave us.
...
**Answer:** The date is now in the correct order -- year, month, date and is separated by dashes instead of slashes. It is also a date column now.
...
The code takes all the expenditures from the month and assigns it to the first day of each month from 2021-2022.
...
**Answer** The first line of code is assigning all of the following code to "general_22" and then calling the original general_22 "and then" -- The second line of code is calling to add two columns. The first one is called total_votes and will consist of votes for all the candidates and write_ins. The second column is called pct_more and divides moore's votes by the total votes.
...
**Answer** It's now arranged by the county that had the highest percentage of votes for Moore, descending to lowest.
...
**Answer** I think it depends on what you're looking for. I'm not familiar with Maryland counties as someone from out of state, but if it's a county that is pretty Democratic and has one of the lowest percentage of votes for Moore, that could be interesting. If possible, it would be nice to move the percentage column to the front or just only show the percentage column with the select function.
...
**Answer** str_to_lower() converts to lower case, str_to_title() converts to title case, or when only the first letter of each word is capitalized.
...
**Answer** The code is adding another column that shows which state the payee is located in based off of the address. When the code detects "Maryland" in the address column, for example, it's instructed to put "MD" in the appropriate row under the state column. If a state isn't detected, add N/A instead.
...
**Answer** The total isn't too specific and that's a bit confusing. I believe it's the amount of money directed into each state, which can be helpful but still not specific to any one payee. Could be more useful if sorted into "in_maryland," "outside_maryland" and "N/A."
...
1. How much the money was paid within Maryland? Outside?

2. What percentage of payee names are unaccounted for?

3. How often was Firehouse Subs logged as an expense?
