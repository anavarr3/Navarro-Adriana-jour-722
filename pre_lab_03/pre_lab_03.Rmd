...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** character, numbers (dbl), character
...
**Answer** 550 zipcodes don't have 5 characters in length -- it's possible they ended in 0 and the character type dbl might have excluded those zeroes
...
**Answer** md_grants_loands |>  is pulling up the data and then -- setting the columns as 'Fiscal Year' and Category and then -- adding a count column, arranging by 'Fiscal Year.'

In the category column, some of the rows entries are empty. Also not a huge fan of their only being a "1" or another small number in most of those counts while they're in the hundreds for others.
...
**Answer** The smallest amount is $60 from the Land Preservation Trust and the next smallest is $95.98 from the United Cerebral Palsy of Central Maryland Inc.-- both odd amounts for grants. 
...
**Answer** It lowercases the column names and adds underscores in the column names where spaces once were.
...
**Answer** creates new database called cleaned_md)grants_loans by pulling md_grants_loans AND THEN, cleans the names as shown above AND THEN renames the grantor column as source AND THEN adjusts the columns: source to all upercase, grantee to all uppercase and description to all uppercase. Then it displays the dataset.
...
**Answer** There's a new column at the end called zip5 that shows 5-digit zip codes.
...
**Answer** I got two results: 21202 (Baltimore) and 21601 (Easton, MD). Baltimore makes sense if the MD Tourism Board is encouraging tourism around the MD capitol. I'm not terribly familiar with Easton but from a quick Google search it looks like people could visit it for the historic buildings/downtown.
