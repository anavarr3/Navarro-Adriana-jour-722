...
**Answer** It looks like a higher population growth in 2021 was associated with a higher gdp growth
```{r}
...
**Answer** GDP growth to the left and pupils on the x-axis. A blue line trends slightly downward.
```{r}
...
**Answer** It looks like gdp growth and pop change trend together where as the relationship between gdp nd per pupil is negative
...
**Answer** I see a small rundown of the data, a hypothesis, and an estimate of correlations
```{r}
...
**Task** Run the following code. Describe the model output. What do you see? A negative correlation like the last one. Also a high p-value.
**Answer**
```{r}
...
### Task 8: Average data
**Task** Run the following code. Describe the table. What do you see? A dataset that shows the 2020 votes, the mean gdp growth, mean per pupil education and mean pop growth for each of the two candidates. Biden had higher gdp growth, higher per pupil education and lower mean pop growth.
**Answer**
```{r}
state_data |>
  group_by(vote_2020) |>
...
**Answer** A Welch Two Sample t-test that has a p-value of 0.07. It also suggests a different hypothesis: true difference in means between Biden and Trump is not equal to 0.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** It has a p-value of .0001, a stronger p-value than before
```{r}
